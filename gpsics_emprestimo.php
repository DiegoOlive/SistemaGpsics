...
<?php //informações passadas para gpsics_emprestimo_excluir/excluir             
...
<!--    ---------------- SEÇÃO BUSCA ---------------    -->
    ...
    <!--    ---------------- SEÇÃO REALIZAR EMPRÉSTIMO---------------    -->

        <CENTER><h2>Realizar Empréstimo de Equipamento</h2></CENTER>
        <table BORDER=0 WIDTH=50% ALIGN=CENTER>
            ...
            if($pesq == 1){
               ...
               $querySelect = $link->query("select *from assoc_tag where equip_at = '$c_equipamento'");
                ...
                while($quant = $querySelect->fetch_assoc()):
                 // $quant_t = $quant['quant_total'];
                  $quant_e = $quant['quant_par'];
                endwhile;
                
                $rest= ($a_equip_at - $quant_e);
                if($rest==0){
                ?>                     
               <CENTER><div class="mensagem">Não Há Mais Equipamento Desse Tipo </div></CENTER>               
               <?php    
                }else if($rest==1){
                ?>                     
               <CENTER><div class="mensagem">Quantidade Excedida! Resta Apena <?php echo $rest; ?> Unidade </div></CENTER>               
               <?php    
                }else{
               ?>                     
               <CENTER><div class="mensagem">Quantidade Excedida! Restam Apenas <?php echo $rest; ?> Unidades </div></CENTER>               
               <?php               
            }}
            
            if($pesq == 3){                
                ?>
                  <CENTER><div class="mensagem">Tag Inválida!</div></CENTER>                 
                <?php                
            }
            ?>
            ...
            <form action = "<?php echo $_SERVER['PHP_SELF']; ?>" method="POST"> 
                    <td><h2>ID da Tag: 
                        <input  name="c_idtag" value="" required type="int">
                    </h2></td>
                    <td><h2>Equipamento:
                        <select name="c_equipamento">
                            <option>--equipamento--</option>
                            <?php                             
	                            $r_equipamento = "SELECT * FROM equipamentos";	
	                            $resultado_equip=mysqli_query($connect, $r_equipamento);
	                        While($row_equipamentos = mysqli_fetch_assoc($resultado_equip)){?>
	                            <option value="<?php echo $row_equipamentos['equipamento'];?>">
                                <?php echo $row_equipamentos['equipamento'];?>
	                            </option><?php
                            }?>
                        </select>  
                    </h2></td>
                    <td><h2>Quantidade: 
                        <input  name="c_quantidade" value="" required type="int">
                    </h2></td>
                    <td>
                    <input type="submit" name="salvar" value="" title="Salvar" class="bt_salvar" style="cursor: pointer">
                    </td>                                          
            </form>
            <tr>                
        </table>
        ...
        <table BORDER=1 WIDTH=95% ALIGN=CENTER>            
            ...
            <?php
            $id_tirar = $_SESSION['id_at'];
            $quant_tirar = filter_input(INPUT_GET, 'quant_at', FILTER_SANITIZE_NUMBER_INT); 
            ?>    
                <tr>                    
                   ...
                   <td><CENTER>
                   <?php $id_at=$linha['id_at'];                      						 
                       echo "<a name='id_at' href='gpsics_emprestimo_editar.php?id_at=$id_at'><i class='material-icone'>Editar</i>"
                   ?> 
                    </CENTER></td>  
                </tr>
